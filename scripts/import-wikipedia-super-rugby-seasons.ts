/**
 * Bulk import Super Rugby seasons from Wikipedia (tables, fixtures, playoffs).
 *
 * Prefer Planet Rugby / SDMS feeds first for recent seasons, then run this
 * to fill historical gaps (Super 12 1996 → Super Rugby Pacific).
 *
 * Usage:
 *   npx tsx --require ./scripts/stub-server-only.cjs scripts/import-wikipedia-super-rugby-seasons.ts --audit
 *   npx tsx --require ./scripts/stub-server-only.cjs scripts/import-wikipedia-super-rugby-seasons.ts --gaps-only
 *   npx tsx --require ./scripts/stub-server-only.cjs scripts/import-wikipedia-super-rugby-seasons.ts --from=1996
 *   npx tsx --require ./scripts/stub-server-only.cjs scripts/import-wikipedia-super-rugby-seasons.ts --year=2011
 *   npx tsx --require ./scripts/stub-server-only.cjs scripts/import-wikipedia-super-rugby-seasons.ts --all
 */
...
import {
  ...
  superRugbyWikipediaSeasonUrls,
} from "../apps/web/src/lib/wikipedia-season-import-service";

const COMPETITION_SLUG = "super-rugby";
const WIKIPEDIA_URL = "https://en.wikipedia.org/wiki/Super_Rugby";
...
const fromYear = Number.parseInt(args.find((a) => a.startsWith("--from="))?.split("=")[1] ?? "1996", 10);
...
async function fixCalendarYearLabels(competitionId: string) {
  ...
  if (rows.length) {
    console.log(`Fixed ${rows.length} season label(s) to calendar year`);
  }
}

async function main() {
  console.log("=== Super Rugby Wikipedia import ===");
  ...
  let seasons = superRugbyWikipediaSeasonUrls().filter((s) => s.startYear >= fromYear);
  ...
  console.log(`\n→ Super Rugby: ${seasons.length} season(s) to import`);
  if (auditOnly) {
  ...
  console.log("Catalog size:", superRugbyWikipediaSeasonUrls().length);
}
