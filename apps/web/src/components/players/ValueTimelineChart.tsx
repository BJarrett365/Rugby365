...
export function ValueTimelineChart({
  ...
  if (!timeline.length) return <p className="pr-mc-transfers-muted">No value timeline yet.</p>;
  if (timeline.length < 2) return <p className="pr-mc-transfers-muted">Insufficient value history</p>;
  ...
  const pointLabel = (t: (typeof timeline)[number]) => {
    const anyT = t as unknown as { asOfDate?: string | null; snapshotAt?: string | null; date?: string | null };
    const iso = anyT.asOfDate ?? anyT.snapshotAt ?? anyT.date ?? null;
    if (iso) {
  ...
  const showAllLabels = timeline.length <= 6;
  ...
  return (
    <figure className={`pr-value-timeline ${className ?? ""}`.trim()}>
      <svg viewBox={`0 0 ${w} ${h}`} className="pr-value-timeline__svg" role="img" aria-label="Market value timeline">
        ...
        <path d={`${path} L ${points[points.length - 1]!.x} ${h - pad} L ${points[0]!.x} ${h - pad} Z`} fill="url(#prValueLine)" />
        ...
        {showAllLabels
          ...
          : (() => {
              ...
              return (
                <>
                  <span>
                    <em>{pointLabel(first)}</em>
                    <strong>{formatGbpCompact(first.marketValueGbp)}</strong>
                  </span>
                  <span aria-hidden>…</span>
                  <span>
                    <em>{pointLabel(last)}</em>
                    <strong>{formatGbpCompact(last.marketValueGbp)}</strong>
                  </span>
                </>
              );
            })()}
