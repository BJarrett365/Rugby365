...
function TeamLeaderboardCard({
  ...
  board: CompetitionTeamLeaderboardBoard;
  expanded: boolean;
  onToggle: () => void;
}) {
  ...
  return (
    <article className="stat-board stat-board--team">
      ...
      {board.entries.length === 0 ? (
        ...
        <ol className="stat-board__list">
          {visible.map((entry) => (
            <li key={`${board.metric}-${entry.teamId}`} className="stat-board__row stat-board__row--team">
              <span
                className={`stat-board__rank${entry.rank <= 3 ? " stat-board__rank--podium" : ""}`}
              >
                {entry.rank}
              </span>
              <span className="stat-board__crest">
                <TeamCrest name={entry.teamName} imageUrl={entry.teamImageUrl} size="sm" />
              </span>
              <span className="stat-board__identity">
                <Link href={`/teams/${entry.teamSlug}`} className="stat-board__name stat-board__name--team">
                  {entry.teamName}
                </Link>
              </span>
...
export function CompetitionTeamStatsClient({
  slug,
  initialSeason,
  ...
  const [data, setData] = useState<CompetitionTeamStatsPayload | null>(null);
    ...
    <div className="competition-stats">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 mb-4">
        <p className="text-sm text-zinc-500 m-0">
          ...
          {data ? ` · ${data.coverage.teamCount} teams · ${data.coverage.rowCount} match rows` : ""}
        </p>
      ...
      {loading ? (
        <p className="text-zinc-500 text-sm">Loading team stats…</p>
      ) : error ? (
        ...
        <>
          ...
          <div className="mb-3 flex items-center justify-between gap-3">
            <h2 className="text-base font-semibold m-0">More team stats</h2>
            <button
              type="button"
              className="cms-btn cms-btn--secondary text-xs"
          ...
          {showAdditional ? (
            ...
            <p className="text-sm text-zinc-500 m-0">
              Also available: turnovers won, clean breaks, defenders beaten, conversions, and
              penalties.
            </p>
          )}
